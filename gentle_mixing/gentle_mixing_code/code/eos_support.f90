...
contains
   ...
   subroutine solve_eos_given_DP( &
      ...
      integer :: eos_calls
      real(dp) :: eos_x, eos_z
      real(dp) :: dlogT, logT_bnd1, logT_bnd2
      ...
      ! for cpm, we roughly now the temperature range.
      if (s% fix_eos_result /= gentle_mixing_default_modes) then
         dlogT = 0.5d0
         logT_bnd1 = max(2d0,logT_guess-dlogT)
         logT_bnd2 = min(6d0,logT_guess+dlogT)
      else
      ...
      call eosDT_get_T( &
         s% eos_handle, &
         s% species, s% chem_id, s% net_iso, xa, &
         logRho, i_logPtot, logP, logT_tol, logP_tol, MAX_ITER_FOR_SOLVE,&
   ...
   ! Solve for temperature & eos results data given gas pressure &
   ! entropy

   subroutine solve_eos_given_PgasS( &
      s, k, xa, &
      logPgas, logS, logT_guess, logT_tol, logS_tol, &
      logRho, logT, res, dres_dlnRho, dres_dlnT, dres_dxa, &
      ierr)

      use eos_def
      use eos_lib, only: eosPT_get_T

      type (star_info), pointer :: s
      integer, intent(in) :: k ! 0 indicates not for a particular cell.
      real(dp), intent(in) :: &
         xa(:), logS, logPgas, &
         logT_guess, logT_tol, logS_tol ! all logs in log10
      real(dp), intent(out) :: logRho, logT
      real(dp) :: dlnRho_dlnPgas_const_T, dlnRho_dlnT_const_Pgas, Rho, logT_bnd1, logT_bnd2, lnS_bnd1, lnS_bnd2, dlogT
      real(dp), dimension(num_eos_basic_results), intent(out) :: &
         res, dres_dlnRho, dres_dlnT
      real(dp), intent(out) :: dres_dxa(num_eos_d_dxa_results,s% species)
      integer, intent(out) :: ierr
      ...
      ! The bounds for logT come from the EoS for planets. They need to be modified for stars.
      ! I (Henrik) never saw the temperature jump more than about 0.3 in simulations. So I set the bounds to 0.4
      ! Make sure the bounds lie within the range of the EoS
      dlogT = 0.4d0
      ! logT_bnd1 = arg_not_provided
      logT_bnd1 = max(2d0,logT_guess-dlogT)  
      ! logT_bnd2 = arg_not_provided
      ...
      ! Get boundaries for the entropy
      lnS_bnd1 = arg_not_provided !min(s%lnS)
      lnS_bnd2 = arg_not_provided !max(s%lnS)

      call eosPT_get_T( &
         s% eos_handle, &
         s% species, s% chem_id, s% net_iso, xa, &
         logPgas, i_lnS, logS*ln10, &
         logT_tol, logS_tol*ln10, MAX_ITER_FOR_SOLVE, &
         logT_guess, logT_bnd1, logT_bnd2, &
         lnS_bnd1, lnS_bnd2, &
         logT, Rho, logRho, &
         dlnRho_dlnPgas_const_T, dlnRho_dlnT_const_Pgas, &
      ...
      if (logRho < -25d0) ierr = -1

      if (ierr /= 0 .and. s% report_ierr) then
         write(*,*) 'Call to eosPT_get_T failed in solve_eos_given_PgasS'
         write(*,2) 'logPgas', k, logPgas
         write(*,2) 'logS', k, logS
   ...
   end subroutine solve_eos_given_PgasS
