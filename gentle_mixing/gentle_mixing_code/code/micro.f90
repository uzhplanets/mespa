...
contains
  ...
  subroutine do_eos_for_cell(s,k,ierr)
    ...
    real(dp) :: logRho_out, logT_out, logS

    integer, pointer :: net_iso(:)
    integer :: j, species, i_var, i_var_sink
    real(dp), parameter :: epsder = 1d-4, Z_limit = 0.5d0
    real(dp), parameter :: LOGRHO_TOL = 1d-8, LOGPGAS_TOL = 1d-8, LOGT_TOL = 1d-8, LOGS_TOL = 1d-8
   ...
   ! case 2: PD mode
   else if (s% fix_eos_result == gentle_mixing_PD_MODE) then
       logPgas = s% lnPgas(k)/ln10
       logRho = s% lnd(k)/ln10

       call solve_eos_given_DP( &
       s, k, s% xa(:,k), &
       logRho, logPgas, logT, LOGT_TOL, LOGPGAS_TOL, &
       logT_out, res, d_dlnd, d_dlnT, d_dxa, &
       ierr)

       if (ierr /=0) then
          if (s% report_ierr) then
             write(*,*) 'do_eos_for_cell: solve_eos_given_DP ierr', ierr
             !call mesa_error(__FILE__,__LINE__,'do_eos_for_cell')
          end if
          return
       end if

       s% lnT(k) = logT_out*ln10
       s% T(k) = exp(s% lnT(k))
       logT = s% lnT(k)/ln10
         
   ! case 3: PS mode
   else if (s% fix_eos_result == gentle_mixing_PS_MODE) then
         
         logPgas = s% lnPgas(k)/ln10
         logS = s% lnS(k)/ln10

         call solve_eos_given_PgasS( &
         s, k, s% xa(:,k), &
         logPgas, logS, logT, LOGT_TOL, LOGS_TOL, &
         logRho_out, logT_out, res, d_dlnd, d_dlnT, d_dxa, &
         ierr)

         if (ierr /= 0) then
            if (s% report_ierr) then
               write(*,*) 'do_eos_for_cell: solve_eos_given_PgasS ierr', ierr
               !call mesa_error(__FILE__,__LINE__,'do_eos_for_cell')
            end if
            return
         end if

         s% lnd(k) = logRho_out*ln10
         s% rho(k) = exp(s% lnd(k))
    ...
    ! restore entropy for self-consistency too
    s% lnS(k) = res(i_lnS)
